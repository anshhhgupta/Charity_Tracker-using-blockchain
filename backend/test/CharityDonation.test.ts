...
describe("CharityDonation", function () {
  ...
  describe("Deployment", function () {
    ...
    it("Should initialize with zero values", async function () {
      ...
      expect(stats[1]).to.equal(0); // totalWithdrawn
      expect(stats[2]).to.equal(0); // currentBalance
      expect(stats[3]).to.equal(0); // totalDonations
      expect(stats[4]).to.equal(0); // totalWithdrawals
    });
  });
  ...
  describe("Donations", function () {
    it("Should allow donations with message and anonymity", async function () {
      ...
      const tx = await charityDonation.connect(donor1).donate(message, isAnonymous, {
      ...
      expect(stats[2]).to.equal(donationAmount); // currentBalance
  ...
  describe("Withdrawals", function () {
    ...
    it("Should allow charity to withdraw funds", async function () {
      ...
      const tx = await charityDonation.connect(charityWallet).withdraw(withdrawalAmount, purpose);
      ...
      const stats = await charityDonation.getStats();
      expect(stats[1]).to.equal(withdrawalAmount); // totalWithdrawn
      expect(stats[2]).to.equal(ethers.parseEther("1.0")); // currentBalance
      expect(stats[4]).to.equal(1); // totalWithdrawals
    });
  ...
  describe("Emergency Withdrawals", function () {
    it("Should allow emergency withdrawal when threshold is met", async function () {
      ...
      const tx = await charityDonation.connect(charityWallet).emergencyWithdraw(emergencyAmount);
  ...
  describe("Getters", function () {
    ...
    it("Should return correct recent donations", async function () {
      ...
      expect(recentDonations[0].amount).to.equal(ethers.parseEther("0.5"));
      expect(recentDonations[1].amount).to.equal(ethers.parseEther("0.3"));
    });
